...
#define dataN 500 //数据量500个
    ...
    ////Kmeans数据中心提取
    //YMCV_FVector_Kmeans_GetCenter(myvec, NULL, center, 100);//无labels标签数据分类
    //YMCV_FVector_Kmeans_Classify(myvec, center, labels);//获得数据标签
		//YMCV_Free_Vector_F32_Free(center);
    
    //GMMS训练
    CVGMMS myGMM = YMCV_Creat_Gmms_Creat(myvec->dim, Knum);//创建高斯混合模型
    YMCV_Fdata_GMMs_Learning(myGMM, myvec, labels);//对带标签的数据进行训练
