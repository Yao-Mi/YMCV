...
/*******************************************************************************************
    *
    * 说明  该例程以 图像fft为例，展示了将一幅图像进行高斯高通滤波的结果，
    *
    ***************************************************************************************/
void runYMCV_Demo(CVIMAGE myImage0)
{
    ...
    CVFREQ_RENDER freqKernl = YMCV_Creat_FreqKernel_Render_Creat(CV_GaussHFilter, CV_RhoAndTheta);
    //加载滤波器参数
    CV_freq_1param myparam = { .gauss.D0 = 200 };
    ...
    LCD_show_image(stx + 0, 16, myImage2);
    sprintf(myInform, "gauss high filter  "); //信息显示
    LCD_ShowString(stx + 0, 0, myInform);
