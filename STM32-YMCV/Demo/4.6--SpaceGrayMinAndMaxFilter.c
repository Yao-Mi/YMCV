...
/*******************************************************************************************
    *
    * 说明  该例程以 最小值和最大值滤波为例，展示了一幅图像 的滤波效果
    *
    ***************************************************************************************/
void runYMCV_Demo(CVIMAGE myImage0)
{
    ...
    //加载渲染器参数，便于进行初始化
    CV_kernel_1param myParam;//极大、极小值滤波无需加载
    ...
    for (kernel_r = 1; kernel_r < 10; kernel_r++)
    {
        ...
        //使用参数进行渲染器通道配置、初始化
        YMCV_Space_SingleKernel_Load(myKernel, &myParam);//最小值滤波无需加载，该步骤也可省略
        ...
        //信息显示
        sprintf(myInform, "minf radius: %02d   . ", kernel_r);
    ...
    for (kernel_r = 1; kernel_r < 10; kernel_r++)
    {
        ...
        //使用参数进行渲染器通道配置、初始化
        YMCV_Space_SingleKernel_Load(myKernel, &myParam);//最大值滤波无需加载，该步骤也可省略
        ...
        //信息显示
        sprintf(myInform, "maxf radius: %02d   . ", kernel_r);
        LCD_ShowString(stx + 0,  myImage0_gray->height, myInform);
        //暂停300ms
        Delay(300);
