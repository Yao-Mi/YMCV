...
int main()
{
    ...
    char myInform[100];
    //信息显示
    sprintf(myInform, "AprilTag解码  ");
    LCD_ShowString(400, 410, myInform);
    /*----------------------------------- 解码apriltag ------------------------------------------*/

    CVIMAGE myImage0 = YMCV_Image_Read("D:/Documents/1012_YMCV/图片素材/AprilTag/16h5(4x4)_2.bmp", CVGrayType);
    ...
    {
        CV_apriltags_data* tda = &result->datap[i * result->dim];
