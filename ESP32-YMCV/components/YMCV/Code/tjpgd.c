...
/*---------------------------------------------*/
static inline uint8 BYTECLIP (int val)
{
	if (val < 0) val = 0;
	if (val > 255) val = 255;
...
)
{
	...
	cv_log_explain(rp == NULL, CV_LOG_ParamI, "解码缓存池长度不足");
...
)
{
	uint8 *seg, b;
	uint16 marker;
	uint32 ofs;//原本类型unsiged long
	uint32 n, i, j, len;
	JRESULT rc;



	cv_log_explain(pool == NULL, CV_LOG_PtrI, "解码缓存池pool不存在");
