...
#define LCD_Free(ptr) free(ptr)
// 调试
#define LCD_DebugPrintf(tipStr) printf(tipStr)

uint16_t LCD_W = 240, LCD_H = 240;

// #define LCD_GRAM_Buflen 1 * sizeof(uint16)
#define LCD_GRAM_Buflen 240 * 5 * sizeof(uint16)

uint8_t LCD_GRAM_Buffm[LCD_GRAM_Buflen];
...
void LCD_Clear(uint16 Color)
{
    ...
    for (int i = 0, imax = LCD_W * LCD_H; i < imax; i++)
    {
        ...
        *pbf++ = cl1;
        // LCD_WR_DATA(Color);
        pbflen += 2;
        if (pbflen >= LCD_GRAM_Buflen)
        {
            LCD_WR_Data_Buff(LCD_GRAM_Buffm, pbflen);
            pbf = LCD_GRAM_Buffm;
            pbflen = 0;
        }
    }
    ...
    for (int y = 0; y < ynum; y++)
    {
        ...
        for (int x = 0; x < xnum; x++)
        {
            ...
            for (int hi = 0; hi < 9; hi++)
            {
                ...
                vcolor = (uint16)CVLimitMaxMin(0, vcolor, 255);
                pencolor.R = vcolor;
                pencolor.G = vcolor;
                pencolor.B = vcolor;
