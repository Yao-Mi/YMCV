...
int main()
{
    ...
    CVIMAGE calibrat = YMCV_Image_Read("../../图片素材/GAN-Face/template.jpg", CVRgb16Type);
    //显示原图
    ...
    //读取一副图片
    CVIMAGE PrImg = YMCV_Image_Read("../../图片素材/GAN-Face/0.jpg", CVRgb16Type);
    ...
    while (1)
    {
        ...
        if (LCD_GetChar(&mchr))
        {
            ...
            {
                FILE* recordfile = fopen("../../图片素材/GAN-Face/CalibratingData.txt", "a");//以追加写入方式打开
                uint8 databuff[20];
                //特征点保存
                for (int i = 0; i <= num; i++)
                {
                ...
                sprintf(myInform, "../../图片素材/GAN-Face/%d.jpg", readimgi);
